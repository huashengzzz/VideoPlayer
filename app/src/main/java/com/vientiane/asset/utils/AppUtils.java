...
import com.vientiane.asset.BuildConfig;
...
public final class AppUtils {
    ...
    public static boolean isSingleActivity(Context mContext) {
        ActivityManager systemService = (ActivityManager) mContext.getSystemService(Context.ACTIVITY_SERVICE);
        List<RunningTaskInfo> tasks = null;
        try {
            tasks = systemService.getRunningTasks(1);
        } catch (SecurityException var4) {
            var4.printStackTrace();
        }
        return (tasks != null && !tasks.isEmpty()) && (tasks.get(0)).numRunning == 1;
    }
